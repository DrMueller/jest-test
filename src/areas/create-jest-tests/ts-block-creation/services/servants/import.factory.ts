...
export class ImportFactory{
  public createImportsForRequiredClasses(sutClass: SutClass): Node[] {
    ...
    const importsToCreate = [
      sutImport,
      ...this.getMockImportStatements(sutClass)
    ];

    return importsToCreate.map(imp => {
      const impSpecifiers = imp.importedObectNames.map(io =>  factory.createImportSpecifier(
        false,
        undefined,
        factory.createIdentifier(io)
      ));
      ...
      return factory.createImportDeclaration(
        ...
        factory.createImportClause(
          false,
          undefined,
          factory.createNamedImports(impSpecifiers)
        ),
  ...
  private getMockImportStatements(sutClass: SutClass): ImportStatement[] {
    ...
    for(const mockName of mockNames ?? []){
      sutClass.imports.forEach(impStatement => {
        if (impStatement.importedObectNames.indexOf(mockName) > -1){
